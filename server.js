...
const Masjid = mongoose.model('Masjid', new mongoose.Schema({ name: String, fajar: String, zuhar: String, asar: String, maghrib: String, isha: String, juma: String, elaan: String }));
const Dairy = mongoose.model('Dairy', new mongoose.Schema({ name: String, milk: String, khoya: String, morning: String, evening: String }));
const Carpool = mongoose.model('Carpool', new mongoose.Schema({ driver: String, vehicle: String, route: String, time: String, seats: String, contact: String }));
...
makeRoute('masjids', Masjid); makeRoute('dairies', Dairy); makeRoute('carpools', Carpool);
...
app.get('/api/weather', (req, res) => res.json({ weather: "Kohlowala ka mosam bilkul theek aur saaf hai." }));

// Global Delete Mapping
const modelsMap = {
  doctors: Doctor, rent: Rent, cargo: Cargo, livestock: Livestock, blood: Blood, rishta: Rishta,
  tubewells: Tubewell, complaints: Complaint, gallery: Gallery, emergency: Emergency, olx: Olx, 
  events: Event, hunar: Hunar, jobs: Job, committees: Committee, masjids: Masjid, dairies: Dairy, carpools: Carpool
};
app.delete('/api/admin/:collection/:id', async (req, res) => {
  ...
  else res.status(400).json({ error: "Invalid context" });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`🚀 Kohlowala Engine online on port ${PORT}`));
